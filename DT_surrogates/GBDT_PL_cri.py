import argparse
import os
import random

import numpy as np
import pandas as pd
import joblib

from sklearn.model_selection import KFold, GroupKFold
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
from sklearn.multioutput import MultiOutputRegressor
from sklearn.metrics import r2_score, mean_absolute_error

import lightgbm as lgb  # GBDT-PL implementation


# ---------- Hyperparameters & Reproducibility ----------
SEED       = 42
KF_SPLITS  = 5
USE_GROUPS = False    # set True to group by Trial
BATCH_SIZE = 512      # placeholder for batch-size (not used by GBDT-PL)

np.random.seed(SEED)
random.seed(SEED)

try:
    import torch
    torch.manual_seed(SEED)
except Exception as e:
    print("[Info] torch seed skipped:", e)


# ---------- CLI Argument Parser ----------
def get_args():
    """
    Parse command-line arguments.

    Returns:
        args with attributes:
            data: path to input CSV
            group_col: column name for grouping folds
            model_out: path for saving final model
    """
    p = argparse.ArgumentParser("5-fold CV for GBDT-PL surrogate")
    p.add_argument("-d", "--data", default="all_curves.csv",
                   help="CSV path (with TMB, HRP, H2O2, t90, I_max, tail_pct)")
    p.add_argument("-g", "--group_col", default="Trial",
                   help="Column name for grouping when USE_GROUPS=True")
    p.add_argument("-o", "--model_out", default="gbdtpl_surrogate_full.joblib",
                   help="Output path for final model fitted on all data")
    return p.parse_args()


# ---------- Load Dataset ----------
def load_dataset(path: str, group_col: str):
    """
    Load features, targets, and optional group labels from CSV.

    Args:
        path: CSV file path.
        group_col: column for grouping folds.

    Returns:
        X: array of shape (n_samples, 3)
        y: array of shape (n_samples, 3)
        groups: array of group labels or None
    """
    if not os.path.isfile(path):
        raise FileNotFoundError(f"{path} does not exist")
    df = pd.read_csv(path)
    X = df[["TMB", "HRP", "H2O2"]].values
    y = df[["t90", "I_max", "tail_pct"]].values
    groups = df[group_col].values if USE_GROUPS else None
    return X, y, groups


# ---------- Build Model Pipeline ----------
def build_model():
    """
    Construct a Pipeline with StandardScaler and LightGBM GBDT-PL.

    Returns:
        sklearn Pipeline with a MultiOutputRegressor wrapper.
    """
    base = lgb.LGBMRegressor(
        objective        = "regression",
        n_estimators     = 800,
        learning_rate    = 0.05,
        subsample        = 0.8,
        colsample_bytree = 0.8,
        num_leaves       = 64,
        reg_lambda       = 1.0,
        linear_tree      = True,     # enable Piece-wise Linear leaves
        random_state     = SEED,
        n_jobs           = -1,
        verbosity        = -1        # silence LightGBM logs
    )
    return Pipeline([
        ("scaler", StandardScaler()),          # keep scaling for linear leaves
        ("gbdtpl", MultiOutputRegressor(base))
    ])


# ---------- Cross-Validation Loop ----------
def cross_validate(X: np.ndarray, y: np.ndarray, groups=None):
    """
    Perform K-fold (or GroupKFold) CV and print per-fold metrics.

    Args:
        X: feature matrix
        y: target matrix
        groups: group labels or None

    Returns:
        List of dicts with fold statistics.
    """
    cv = GroupKFold(n_splits=KF_SPLITS) if USE_GROUPS else \
         KFold(n_splits=KF_SPLITS, shuffle=True, random_state=SEED)

    fold_stats = []
    for fold, (tr, te) in enumerate(cv.split(X, y, groups), 1):
        model = build_model()
        model.fit(X[tr], y[tr])
        pred = model.predict(X[te])

        stats = {
            "fold": fold,
            "t90_r2"   : r2_score(y[te][:, 0], pred[:, 0]),
            "t90_mae"  : mean_absolute_error(y[te][:, 0], pred[:, 0]),
            "imax_r2"  : r2_score(y[te][:, 1], pred[:, 1]),
            "imax_mae" : mean_absolute_error(y[te][:, 1], pred[:, 1]),
            "tail_r2"  : r2_score(y[te][:, 2], pred[:, 2]),
            "tail_mae" : mean_absolute_error(y[te][:, 2], pred[:, 2]),
        }
        fold_stats.append(stats)
        print(f"[Fold {fold}] "
              f"t90_R2={stats['t90_r2']:.3f}  "
              f"Imax_R2={stats['imax_r2']:.3f}  "
              f"Tail_R2={stats['tail_r2']:.3f}")

    # summarize mean ± std for each metric
    def mean_std(lst, key):
        vals = np.array([d[key] for d in lst])
        return vals.mean(), vals.std(ddof=1)

    print("\n===== 5-fold CV Results (mean±sigma) =====")
    for tgt, lbl in zip(["t90", "imax", "tail"], ["t90", "I_max", "tail_pct"]):
        r2_m, r2_s = mean_std(fold_stats, f"{tgt}_r2")
        mae_m, mae_s = mean_std(fold_stats, f"{tgt}_mae")
        print(f"{lbl:<8}  R²={r2_m:.3f}±{r2_s:.3f}   "
              f"MAE={mae_m:.3f}±{mae_s:.3f}")

    return fold_stats


# ---------- Main Entry Point ----------
def main():
    """
    Execute script:
    - parse CLI args
    - load dataset
    - run cross-validation
    - (optional) train and save final model on all data
    """
    args = get_args()
    X, y, groups = load_dataset(args.data, args.group_col)
    cross_validate(X, y, groups)


if __name__ == "__main__":
    main()
