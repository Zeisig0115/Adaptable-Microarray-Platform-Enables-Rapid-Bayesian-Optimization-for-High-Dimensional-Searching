...
from fit_model import fit_saasbo, fit_gp, fit_fullyb_gp, fit_saas_gp, fit_prf, fit_mlp_ensemble
from metrics import (
    point_metrics,
...
def load_numeric_table_no_noise(
    ...
    # FEATURE_WHITELIST = [
    #     "power", "time", "ar", "n2", "pressure", "voltage", "current", "ri"]
...
def fold_eval_learn_noise(
    ...
    warmup: int, num_samples: int, thinning: int, seed: int, crps_T: int,
    model_type: str = "gp",
    device: str = "auto",
) -> FoldResult:
    # 划分
    ...
    return FoldResult(rmse=rmse, r2=r2, nll=nll, msll=msll, crps=crps, n_test=Xte.shape[0])
...
def main():
    ...
    parser.add_argument("--model", type=str, choices=["gp", "saasbo", "fullyb_gp", "saas_gp", "prf", "mlp"], default="prf")
    ...
    for i, (tr_idx, te_idx) in enumerate(splits, 1):
        ...
        res = fold_eval_learn_noise(
            ...
            idx_train=tr_idx, idx_test=te_idx,
            warmup=args.warmup_steps, num_samples=args.num_samples,
            thinning=args.thinning, seed=args.seed + i, crps_T=args.crps_T,
            model_type=args.model,
            device=args.device
        )
        end_time = time.time()
        print(f"[Fold {i}] RMSE={res.rmse:.6f}  R2={res.r2:.4f}  NLL={res.nll:.6f}  MSLL={res.msll:.6f}  CRPS={res.crps:.6f}")
        all_res.append(res)
