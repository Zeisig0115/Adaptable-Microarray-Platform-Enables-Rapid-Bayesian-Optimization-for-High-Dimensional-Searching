...
# 若你已经定义过 DiagNormalPosterior，就复用它；否则保留这个简版实现
class DiagNormalPosterior:
    def __init__(self, mean: torch.Tensor, var: torch.Tensor):
        self._mean = mean
        self._var = var
        self.distribution = None  # 走非-mixture路径
    @property
    def mean(self) -> torch.Tensor: return self._mean
    @property
    def variance(self) -> torch.Tensor: return self._var
    @torch.no_grad()
    def rsample(self, sample_shape: torch.Size = torch.Size()):
        eps = torch.randn(*sample_shape, *self._mean.shape,
                          dtype=self._mean.dtype, device=self._mean.device)
        return self._mean + eps * torch.sqrt(self._var.clamp_min(1e-12))
...
class NGBoostModel:
    """
    NGBoost 回归包装：使用 ngboost 的 pred_dist(X) 得到每点的预测分布，
    再抽取 mean/var 构造对角高斯后验，满足你的评估接口。
    """
    def __init__(self, ngb, device: torch.device):
        self.ngb = ngb
        self.device = device
    ...
    def posterior(self, X: torch.Tensor, observation_noise: bool = True):
        X_np = X.detach().cpu().numpy()
        dists = self.ngb.pred_dist(X_np)

        mu = None
        var = None

        params = getattr(dists, "params", None)
        if isinstance(params, dict) and ("loc" in params):
            mu = np.asarray(params["loc"]).reshape(-1, 1)
            if "scale" in params:
                var = (np.asarray(params["scale"]) ** 2).reshape(-1, 1)

        if mu is None:
            mean_attr = getattr(dists, "mean", None)
            mu = mean_attr() if callable(mean_attr) else np.asarray(mean_attr)
            mu = np.asarray(mu).reshape(-1, 1)
        if var is None:
            var_attr = getattr(dists, "var", None)
            var = var_attr() if callable(var_attr) else np.asarray(var_attr)
            var = np.asarray(var).reshape(-1, 1)

        mean_t = torch.as_tensor(mu, dtype=torch.double, device=self.device)
        var_t = torch.as_tensor(var, dtype=torch.double, device=self.device)
        return DiagNormalPosterior(mean_t, var_t)
